...
B = [
     ...
     8 17   16 15  d0 alpha
    16  9   22 17  d0 alpha
    26 22   26 10  d0 alpha   
];

[P, reached] = Path_generator(xs, xf, eta, B);

figure; hold on; grid on; axis equal;
title('Part (b) - With Extra Obstacle');
xlabel('x'); ylabel('y');
...
if reached
    disp('Reached the goal (unlikely with local minima, but possible).');
else
    disp('Stuck (local minimum) or maxIter reached.');
    disp(['Stuck near: [', num2str(P(end,1)), ', ', num2str(P(end,2)), ']']);
end
